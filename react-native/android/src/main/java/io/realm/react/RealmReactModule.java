...
class RealmReactModule extends ReactContextBaseJavaModule {
    ...
    static {
        try {
            Log.i("Realm", "Loading the module");
            SoLoader.loadLibrary("realm");
            Log.i("Realm", "Loaded the module");
    ...
    public void initialize() {
        Log.i("Realm", "in init");
        JavaScriptContextHolder jsCtx = ctx.getJavaScriptContextHolder();
        synchronized(jsCtx) {
            Log.i("Realm", String.valueOf(jsCtx.get()));
            init(jsCtx.get());
            Log.i("Realm", "completed init");
        }
    }
    ...
    private native void init(long runtime);
